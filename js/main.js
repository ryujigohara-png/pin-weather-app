...
const defaultViewConfig = {
    ...
    themeMode: 'system',           // テーマモード ('system' or 'light' or 'dark')
    displayMode: 'graph',          // 【追加】初期表示モード ('text' or 'graph')
...
const i18n = {
    ...
    dict: {
        'ja': {
            ...
            guideLabel5: "新着予報のプッシュ通知",
            guideBody5: "登録した地点の12時間分の予報テキストを、指定した時刻（1地点につき最大3回）にスマホへお届けします。通知をタップするとPWAが起動し、その場所の予報へダイレクトにアクセスできます。<br><br><strong>設定方法：</strong><br>・毎日の通知設定から、通知を受け取りたい時刻を指定して有効にします。",
            guideLabel6: "2つの表示モード（グラフ表示 / 文字表示）",
            guideBody6: "左上のボタンから、目的に合わせて2つの画面をいつでも切り替えられます。<br>・<strong>グラフ表示：</strong> 風向・風速・気温に加え、波高や潮位、詳細な数値データまで全てを網羅した高機能画面です。<br>・<strong>文字表示：</strong> 3時間ごとの天気・気温・風向風速の要点を、シンプルに一覧で素早く確認したい時に最適です。",
        ...
        'en': {
            ...
            guideLabel5: "Daily Forecast Push Notification",
            guideBody5: "Receive a 12-hour forecast text for registered spots directly on your phone at your preferred times (up to 3 slots per spot). Tap the notification to open the PWA and access the latest updates instantly.<br><br><strong>How to set:</strong><br>・Go to Daily Notification, specify your preferred notification times, and enable the feature.",
            guideLabel6: "Two Display Modes (Graph / Text View)",
            guideBody6: "Switch between two screens anytime using the button in the upper-left corner.<br>・<strong>Graph View:</strong> A full-featured screen packed with detailed data including wind speed/direction, temperature, wave height, tide, and comprehensive numeric metrics.<br>・<strong>Text View:</strong> Best for a quick glance to check the core forecast data for weather, temperature, and wind speed/direction every 3 hours in a simple list format.",
 ...
 * 修正内容：ブラウザ言語設定を判定し、日本語なら日本語地名、それ以外なら英語地名を取得。
 * 「(Approx)」等の識別子を削除。
 */
async function setApproximateLocation() {
    try {
        // 1. 無料のIP Geolocation API (ipapi.co) で座標を取得
        ...
        if (data.latitude && data.longitude) {
            ...
            // 2. 言語判定と逆引きジオコーディング
            try {
                // ブラウザの言語設定を判定（日本語かどうか）
                const isJa = i18n._currentLang === 'ja';
                const targetLang = isJa ? 'ja' : 'en';
                
                const revRes = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${currentLat}&lon=${currentLon}&format=json&accept-language=${targetLang}`);
                ...
                if (revRes.ok) {
                    ...
                    // 優先順位に基づいて地名を抽出
                    let locName = addr.city || addr.town || addr.village || addr.province || addr.state || data.city || "現在地";
                    
                    // 識別記号を付けずに代入
                    currentLabel = locName;
                } else {
                    // 逆引き失敗時はipapi.coのデータを使用（識別記号なし）
                    currentLabel = data.city || data.region || "現在地";
                }
            } catch (revErr) {
                console.warn("Reverse geocoding failed, using IP city name.", revErr);
                currentLabel = data.city || data.region || "現在地";
            }
        } else {
 ...
 * 修正内容：既存の.btn-gpsに加え、新設された#gps-btn-navの状態も更新。
 */
function handleGPSClick() {
    if ("geolocation" in navigator) {
        const gpsBtns = document.querySelectorAll('.btn-gps, #gps-btn-nav');
        gpsBtns.forEach(btn => { if(btn) btn.innerText = i18n.t('gpsFetching'); });
        ...
        navigator.geolocation.getCurrentPosition(async (pos) => {
            ...
            gpsBtns.forEach(btn => { if(btn) btn.innerText = "GPS"; });
